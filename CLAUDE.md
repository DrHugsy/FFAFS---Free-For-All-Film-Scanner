...
- Ícono de marca / favicon: un fantasmita simple de trazo blanco (dos ojos redondos + sonrisa), compartido igual en las 6 páginas. Es el placeholder de la mascota "Iggy" — el usuario lo va a redibujar él mismo; cuando lo haga, hay que reemplazar el SVG en las 6 páginas (buscá el `<svg class="mascot"` en `index.html` para la versión grande, y el `<svg viewBox="0 0 32 32">` dentro de `.brand` / el `<link rel="icon">` en cada página para la versión chica). En `index.html` el mascot grande está animado (float suave + parpadeo periódico de los ojos vía CSS `@keyframes`, clases `.mascot`/`.mascot-eye`) — el usuario pidió específicamente animarlo ("si podes animar al fantasmita genial"). Si se redibuja, tratar de mantener esa animación o una equivalente.
...
- **Menú hamburguesa**: las 6 páginas tienen un botón `#btnMenu` (ícono de 3 líneas) a la izquierda de la marca, que abre un `.nav-menu` desplegable con links a Home + las 4 herramientas + Terms, con la página actual resaltada (`class="current"`, fondo blanco/texto negro). Pedido explícito del usuario ("arriba a la izquierda... un menu desplegable"). El patrón (`.hamburger`, `.nav-menu`, `.menu-backdrop`, más el JS `toggleMenu()`/cierre con click-afuera/Escape) es idéntico en las 6 páginas — copialo tal cual si agregás una herramienta nueva. **Ojo**: un `<button>` no hereda `color` del body por defecto en algunos navegadores — si agregás un botón con ícono SVG `stroke="currentColor"`, ponele `color` explícito en su CSS o el ícono queda invisible (pasó una vez con `.hamburger` en `index.html`).
...
3. El usuario puede pedir seguir sumando herramientas al hub — el patrón para agregar una nueva es: crear el `.html` con el mismo esqueleto visual (copiá el `<style>` de `:root` en adelante, el nav con menú hamburguesa, y la mascota de cualquier página existente — `convert.html` es la más reciente y prolija para copiar — ver "Identidad visual"), agregar su tarjeta en `index.html` **y** su link en el `.nav-menu` de las 6 páginas (index, terms, cuarto-oscuro, photo-editor, lut-lab, convert), y si necesita RAW o cualquier otro endpoint de servidor, reusar `server_prod.py` en vez de crear un backend nuevo.
